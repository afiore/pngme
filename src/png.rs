...
impl Png {
    ...
    pub(crate) fn append_chunk(&mut self, chunk: Chunk) {
        let last_idx = std::cmp::max(0, self.chunks.len() - 1);
        self.chunks.insert(last_idx, chunk);
    }
    ...
    pub(crate) fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, ()> {
        let idx = self.chunks.iter().enumerate().find_map(|(idx, chunk)| {
        ...
        idx.map(|idx| self.chunks.remove(idx)).ok_or_else(|| ())
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        Png::try_from(&mut reader)
    }
}

impl<T> TryFrom<&mut BufReader<T>> for Png
where
    T: Read,
{
    type Error = ();

    fn try_from(reader: &mut BufReader<T>) -> Result<Self, Self::Error> {
        let mut chunks = Vec::new();
        let mut parsed_header = [0 as u8; 8];

        reader.read_exact(&mut parsed_header).map_err(|_| ())?;

        if parsed_header != Png::STANDARD_HEADER {
            Err(())
        } else {
            loop {
                ...
                //chunk_type + data + CRC
                let chunk_length = 4 + u32::from_be_bytes(length_bytes) + 4;

                let mut chunk_bytes: Vec<u8> = length_bytes.into();

                let mut chunk_bytes_no_length: Vec<u8> = vec![0 as u8; chunk_length as usize];

                reader
                    .read_exact(&mut chunk_bytes_no_length)
                    .map_err(|_| ())?;

                chunk_bytes.extend(chunk_bytes_no_length);

                let chunk = Chunk::try_from(chunk_bytes.as_ref()).map_err(|_| ())?;
                chunks.push(chunk.clone());

                if chunk.chunk_type().is_iend()
                    || reader
                        .fill_buf()
                        .map_or_else(|_| false, |buf| buf.is_empty())
                {
                    break;
                }
            }
...
mod tests {
    ...
    fn chunk_from_strings(chunk_type: &str, data: &str) -> Result<Chunk, ()> {
        ...
        let chunk_type = ChunkType::from_str(chunk_type).map_err(|_| ())?;
    ...
    fn test_valid_from_bytes() {
        ...
        let x: &[u8] = bytes.as_ref();

        let png = Png::try_from(x);
